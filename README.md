...
用户端 H5 和管理后台现在都已接入真实后端接口，默认请求：`http://localhost:3001/api`。

如需改后端地址，可分别在 `web/.env`、`admin/.env` 中配置：
...
- 检查后端健康状态
- 注册/登录并保存 Bearer token
- 发布问题求助
- 发布农户货源，包含自送价、上门收购价、视频链接
- 浏览已审核教程、问题、货源、收购需求

管理后台已支持：

- 管理员登录并保存 Bearer token
- 查看待审核统计
- 拉取待审教程、老师入驻、货源、收购需求
- 审核通过/拒绝
- 展示防诈骗审核提醒
...
- 图片/视频真实上传
- 细化角色权限：农户只能发自己的货源，老师才能回答问题，收购商才能发需求
